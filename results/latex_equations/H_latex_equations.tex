...
$$ B =\min_{ a,c,d,f,h } \left( \colorbox{c1}{$D$}\left[a,f|c,d\right]+C\left[c,d,f,h\right]\right) $$
$$ C\left[ c,d,f,h \right] =\min_{ g } \left( \colorbox{c0}{$C_{\boxtimes}$}\left[c,d-1,g,h-1\right]\right) $$
$$ \colorbox{c1}{$D$}' \left[a,f | c,d \right] =  \min\begin{cases}\colorbox{c1}{$D$}'[ a , f-1|c,d ], &\text{if } f-1 ,\notin\{ a , c,d \} \\\colorbox{c1}{$D$}[ a+1 , f-1|c,d ]+\Delta G(a,f) &\text{if } \{ a+1 , f-1 \}\cap \{ c,d \}=\emptyset\end{cases}$$
$$ \colorbox{c1}{$D$} \left[a,f | c,d \right] =  \min\begin{cases}\colorbox{c1}{$D$}[ a+1 , f|c,d ], &\text{if } a+1 \notin\{ f , c,d \} \\\colorbox{c1}{$D$}'[ a , f-1|c,d ], &\text{if } f-1 ,\notin\{ a , c,d \} \\\colorbox{c1}{$D$}[ a+1 , f-1|c,d ]+\Delta G(a,f) &\text{if } \{ a+1 , f-1 \}\cap \{ c,d \}=\emptyset\end{cases}$$
\end{document}
