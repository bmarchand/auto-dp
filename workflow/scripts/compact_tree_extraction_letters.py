import sys
import copy
from utils import read_td_lines, full_compact_diag, full_compact_clique, filter_extremities, subgraph_info_extraction
...
index2bag = filter_extremities(all_extremities, adj, index2bag)
    ...
    # clique case
        for c in index2bag[u]:
            if c in set(index2bag[u]).intersection(set(index2bag[prev])):
                ...
                label += '  <FONT COLOR="GREEN">'+boldified(c, all_extremities)+'</FONT>'
    ...
    # other bags
        ...
        for c in index2bag[u]:
            if c in set(index2bag[u]).intersection(set(index2bag[prev])):
                ...
                label += '  <FONT COLOR="DARKGREEN">'+boldified(c, all_extremities)+'</FONT>'
    label += "}>"
